...
save = function () {
	...
	var cls = document.getElementById('my_id');
	window.location.href = '/timetable/timetable_edit/'+cls.value+'/'+content;
};
...
reportButton = function () {
	var	id,			// element id
		...
		// prepare subjects
		subject = {'en':0,'ma':0,'ki':0,'ph':0,'ch':0,'bi':0,'ge':0,'hi':0,'cr':0,'ar':0,'ho':0,'ag':0,'bu':0,'mu':0,'fr':0,'gr':0};
	// loop goes through all collected elements
	for (i = 0; i < divNodeList.length; i++) {
		// define only first two letters of ID
		// (cloned elements have appended c1, c2, c3 ...)
